...
(defvar mcp-ant-home (if (mktop-p) "/opt/ant-1.7.0" "/localdisk/ant"))
...
(defvar mcp-stagedir-default (if (mktop-p) "/home/mk/nt/staging" "/localdisk/data/matthewk/staging"))
    ...
    (let ((viewdir (first lst))
      ...
      (if (and workdir viewdir stagedir)
          (let* ((startup-hook-name (concat name "-startup-hook"))
                 ...
                 (proj-dir (concat (homedir) "/.mk-project/" name "/")))
            ...
            (project-def name
                         `((basedir ,viewdir)
                           ...
                           (file-list-cache ,(concat proj-dir "files"))
                           (open-files-cache ,(concat proj-dir "open-files"))
            ...
            (message "Created MCP project %s\n  viewdir is %s\n  workdir is %s\n  stagedir is %s" name viewdir workdir stagedir))
    ...
    (let (viewdir workdir stagedir)
      ...
      (with-temp-buffer
        ...
        (while (not (eobp))
          ...
          (forward-line)))
      (list (normalize-dirname viewdir) 
            (normalize-dirname workdir) 
            (if stagedir stagedir mcp-stagedir-default))))
